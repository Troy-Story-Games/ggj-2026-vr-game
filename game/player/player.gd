...
const HammerHand = preload("res://game/player/hands/hammer_hand.tscn")
const SpectralHand = preload("res://game/player/hands/spectral_hand.tscn")
signal recenter()
...
func _on_mask_detection_area_body_entered(body: Node3D) -> void:
	if body is HammerMask:
	# right_hand becomes Hammer
		var hand = right_controller.find_child("RightHand")
		hand.hide()
		right_controller.add_child(HammerHand)
	print("Body entered area: ", body)
	if body is SpectralMask:
	# right_hand becomes Spectral
		var hand = right_controller.find_child("RightHand")
		hand.hide()
		right_controller.add_child(SpectralHand)
	print("Body entered area: ", body)
